...
public class InvoiceReaderTest {
	...
	private static InvoiceProcessor invoiceProcessor;
	private static List<Role> roles;

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		roles = new RoleReader(FILE_ROLES).getRoles();
		invoiceProcessor = new InvoiceProcessor(FILE_INVOICES, roles);
	}
	...
	public void testTamanhoDaLista() {
		assertEquals(5, invoiceProcessor.getReports().size());
	}
	...
	public void testValidacaoPrimeiroRegistro() {
		...
		assertEquals(report, invoiceProcessor.getReports().get(0));
	}
	...
	public void testValidacaoSegundoRegistro() {
		...
		assertEquals(report, invoiceProcessor.getReports().get(1));
	}
	...
	public void testValidacaoTerceiroRegistro() {
		...
		report.setCorrect(true);
		assertEquals(report, invoiceProcessor.getReports().get(2));
	}
	...
	public void testValidacaoQuartoRegistro() {
		...
		report.setCorrect(false);
		assertEquals(report, invoiceProcessor.getReports().get(3));
	}
	...
	public void testValidacaoQuintoRegistro() {
		...
		report.setRole(roles.get(0));
		report.setCorrect(false);
		assertEquals(report, invoiceProcessor.getReports().get(4));
	}
}
